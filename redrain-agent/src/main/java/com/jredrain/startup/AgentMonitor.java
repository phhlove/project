...
public class AgentMonitor {
    ...
    public void start(final int port) throws Exception {
        ...
        server.addDisconnectListener(new DisconnectListener() {
            ...
            public void onDisconnect(SocketIOClient client) {
                ...
                if (clients.isEmpty()) {
                    stop = true;
                    logger.info("[redrain]:monitor closed:SessionId @ {},port @ {} ", client.getSessionId(), port);
                    server.stop();
                }
            }
        });
        ...
        server.start();
        stop = false;
        logger.debug("[redrain] server started @ {}", port);
    }
    ...
    public  List<String> getIostat() throws Exception {
        ...
        String result = CommandUtils.executeShell(Globals.REDRAIN_MONITOR_SHELL, "io");
